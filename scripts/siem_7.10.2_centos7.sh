...
# elasticsearch 
wget https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-$ES_VER-x86_64.rpm
rpm -ivh elasticsearch-$ES_VER-x86_64.rpm
systemctl daemon-reload
systemctl enable elasticsearch.service
systemctl start elasticsearch.service
sed -i -r "s|^#cluster.name:.*$|cluster.name: siem|" /etc/elasticsearch/elasticsearch.yml
sed -i -r "s|^#network.host:.*$|network.host: _site_|" /etc/elasticsearch/elasticsearch.yml
...
# kibana
wget https://artifacts.elastic.co/downloads/kibana/kibana-$ES_VER-x86_64.rpm
rpm -ivh kibana-$ES_VER-x86_64.rpm
